...
由于直接从Docker官方仓库拉取镜像可能会遇到网络问题，建议配置国内镜像加速器：
...
1. **配置Docker镜像加速器**（推荐）：如上文第2节所述
2. **手动拉取基础镜像**：
   ```bash
   # 尝试从不同的镜像源拉取
   docker pull registry.docker-cn.com/library/ubuntu:20.04
   # 或
   docker pull hub-mirror.c.163.com/library/ubuntu:20.04
   
   # 如果成功拉取，重新标记镜像
   docker tag registry.docker-cn.com/library/ubuntu:20.04 ubuntu:20.04
   ```
